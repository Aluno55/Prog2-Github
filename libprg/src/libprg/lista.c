...
//cheia, capacidade, vazia?

int tama(list_t* L){return L->tam;}

list_t* sort(list_t* l) {
    if (l->order) return l;
    //bubbles(l->elementar, l->tam);
    //insertion(l->elementar, l->tam);
    //selection(l->elementar, l->tam);
    //merge(l->elementar, 0, l->tam);
    //int* quick (l->elementar, 0, l->tam);)
    ////normalmente fazem baseado em l.tam
    l->order = true;
    return l;
}